 ...
 * @author lcy
 */
public class EpdRouteStationAction {
	...
	public String insert(String security,String parameter) {
		...
		try {
	        ...
	        EpdRoutestation epdRoutestation =(EpdRoutestation)JsonUtil.convertObject(
	        		mapParameter.get("epdRoutestation").toString(),EpdRoutestation.class);
	        ...
	        //3.调用业务
			EpdRoutestation newEpdRoutestation = iNetEpdRouteStation.insert(epdRoutestation, mapConfig);
	        result.put("epdRoutestation", JSONObject.fromObject(newEpdRoutestation).toString());
	...
	public String update(String security,String parameter) {
		...
		try {
			...
			Map mapConfig = (Map)jsonConfig;
	        //3.调用业务
			iNetEpdRouteStation.update(epdRoutestation, mapConfig);
		} catch (UserBusinessException e) {
			e.printStackTrace();
			result.put("ResultCode", "1");
			result.put("Message", e.getMessage());
	...
	public String delete(String security,String parameter) {
		...
		try {
			...
			Map mapConfig = (Map)jsonConfig;
	        //3.调用业务
			iNetEpdRouteStation.delete(epdRoutestations, mapConfig);
		} catch (UserBusinessException e) {
			e.printStackTrace();
			result.put("ResultCode", "1");
			result.put("Message", e.getMessage());
	...
	public String queryByPK(String security,String parameter) {
		...
		try {
	        ...
	        JSONObject jsonParameter = JSONObject.fromObject(strParameter);
	        Map mapParameter = (Map)jsonParameter;
	        String routestationSeq = String.valueOf(mapParameter.get("routestationSeq"));
	        //3.调用业务
			List<EpdRoutestation> epdRoutestations = iNetEpdRouteStation.queryByPK(routestationSeq);
			JSONArray jsonarray = JSONArray.fromObject(epdRoutestations);
			result.put("epdRoutestations", jsonarray.toString());
		} catch (UserBusinessException e) {
			e.printStackTrace();
			result.put("ResultCode", "1");
			result.put("Message", e.getMessage());
	...
	public String queryByRouteSeq(String security,String parameter) {
		...
		try {
	        ...
	        //3.调用业务
			List<EpdRoutestation> epdRoutestations = iNetEpdRouteStation.queryByRouteSeq(routeSeq);
			JSONArray jsonarray = JSONArray.fromObject(epdRoutestations);
			result.put("epdRoutestations", jsonarray.toString());
		} catch (UserBusinessException e) {
			e.printStackTrace();
