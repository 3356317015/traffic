...
public class ItsLinerstationAction {
	...
	public String updateAttribute(String security, String parameter) {
		...
		try {
	        ...
	        List<ItsLinerstation> itsLinerstations = (List<ItsLinerstation>) JsonUtil.convertArray(
	        		mapParameter.get("itsLinerstations").toString(), new ItsLinerstation());
	        ...
	        //3.调用业务
			iNetItsLinerstation.updateAttribute(itsLinerstations, mapConfig);
	...
	public String queryByLinerSeq(String security, String parameter) {
		...
		try {
	        ...
	        //3.调用业务
	        List<ItsLinerstation> itsLinerstations = iNetItsLinerstation.queryByLinerSeq(linerSeq);
			JSONArray jsonarray = JSONArray.fromObject(itsLinerstations);
			result.put("itsLinerstations", jsonarray.toString());
		} catch (UserBusinessException e) {
			e.printStackTrace();
			result.put("ResultCode", "1");
			result.put("Message", e.getMessage());
