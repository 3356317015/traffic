...
public class VmsParameterCauseUi extends Dialog {
	...
	protected VmsParameterCauseUi(Shell shell,GridView gridView,String operType) {
		super(shell);
	...
	private void initData(){
		txtTemplateName.setText("异常原因模板");
			...
			if(1 == buttonId){
				...
				if (null != vmsTemplate){
					...
					if (CommFinal.OPER_TYPE_ADD.equals(operType)){
						VmsTemplate newVmsTemplate = iVmsTemplate.insert(vmsTemplate,CommFinal.initConfig());
						gridView.addRow(newVmsTemplate);
						clearData();
	...
	private void clearData() {
		txtTemplateCn.setText("");
		txtTemplateEn.setText("");
		txtTemplateCn.forceFocus();
	}
	...
	private VmsTemplate validData(){
		...
		vmsTemplate.setTemplateType("2");
		vmsTemplate.setTemplateCode("fault_cause");
		vmsTemplate.setTemplateName("异常原因模板");
		if (CommFinal.OPER_TYPE_UPDATE.equals(operType)){
