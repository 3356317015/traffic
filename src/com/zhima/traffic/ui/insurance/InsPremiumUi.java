 ...
 * @author lcy
 */
public class InsPremiumUi extends Composite {
	private Object obj;
	...
	public InsPremiumUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	private void createDetail(){
		...
		columns.add(new GridColumn("保险费","premium",100));
		columns.add(new GridColumn("票价下限","startPrice",100));
		columns.add(new GridColumn("票价上限","limitPrice",100));
		columns.add(new GridColumn("意外伤害险","amountOne",120));
		columns.add(new GridColumn("意外医疗险","amountTwo",120));
	...
	public void queryMethod(){
		InsPremiumFindUi findUi = new InsPremiumFindUi(this.getShell());
		findUi.setfInsurance(fInsurance);
		findUi.open();
		if (findUi.getBtnId()==1){
			fInsurance = findUi.getfInsurance();
			ThreadWaiting waiting = new ThreadWaiting(this,"queryPremium",new Class[]{},new String[]{});
			waiting.task();
		}
	}
	
	public void queryPremium(){
		try {
			for (int i = 0; i < rights.size(); i++) {
				if (rights.get(i).getRightMethod().equals("queryMethod")){
					...
					int count = iInsPremium.queryCountByCustom(CommFinal.organize.getOrganizeSeq(),fInsurance);
					if (count>0){
						List<InsPremium> premiums = iInsPremium.queryPageByCustom(CommFinal.organize.getOrganizeSeq(),
								fInsurance, start, limit);
						gridView.setDataList(premiums);
					}else{
						gridView.removeAll();
					}
	...
	public void addMethod(){
		InsPremiumEditUi editUi = new InsPremiumEditUi(this.getShell(),gridView,CommFinal.OPER_TYPE_ADD);
		editUi.open();
	}
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridView.getSelection()){
					InsPremiumEditUi editUi = new InsPremiumEditUi(this.getShell(), gridView, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
				}else{
	...
	public void deleteMethod(){
		try{
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		IInsPremium iInsPremium = new ImpInsPremium();
		    		iInsPremium.delete((List<InsPremium>) gridView.getSelections(),CommFinal.initConfig());
