 ...
 * @author lcy
 */
public class InsTickettypeUi extends Composite {
	private Object obj;
	...
	public InsTickettypeUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	public void queryMethod(){
		InsTickettypeFindUi findUi = new InsTickettypeFindUi(this.getShell());
		...
		try {
			for (int i = 0; i < rights.size(); i++) {
				if (rights.get(i).getRightMethod().equals("queryMethod")){
					...
					int count = iInsTickettype.queryCountByCustom(CommFinal.organize.getOrganizeSeq(),
							null);
					if (count>0){
						List<InsTickettype> tickettypes = iInsTickettype.queryPageByCustom(CommFinal.organize.getOrganizeSeq(),
	...
	public void addMethod(){
		InsTickettypeEditUi editUi = new InsTickettypeEditUi(this.getShell(),gridView,CommFinal.OPER_TYPE_ADD);
		editUi.open();
	}
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridView.getSelection()){
					InsTickettypeEditUi editUi = new InsTickettypeEditUi(this.getShell(), gridView, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
				}else{
	...
	public void deleteMethod(){
		try {
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		IInsTickettype iInsTickettype = new ImpInsTickettype();
		    		iInsTickettype.delete((List<InsTickettype>) gridView.getSelections(),CommFinal.initConfig());
