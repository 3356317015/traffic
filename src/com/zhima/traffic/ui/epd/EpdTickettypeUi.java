 ...
 * @author lcy
 */
public class EpdTickettypeUi extends Composite {
	private Object obj;
	...
	public EpdTickettypeUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	private void createDetail(){
		...
		columns.add(new GridColumn("票据类型","category",TraffFinal.ARR_CATEGORY,120));
		columns.add(new GridColumn("票据名称","ticketName",120));
		columns.add(new GridColumn("批次数量","ticketNum",120));
		columns.add(new GridColumn("联单页数","ticketPages",120));
		columns.add(new GridColumn("宽度(mm)","ticketWidth",120));
		columns.add(new GridColumn("高度(mm)","ticketHeight",120));
	...
	public void queryMethod(){
		EpdTickettypeFindUi findUi = new EpdTickettypeFindUi(this.getShell());
		findUi.setfCategory(fCategory);
		findUi.open();
		if (findUi.getBtnId()==1){
			fCategory = findUi.getfCategory();
			ThreadWaiting waiting = new ThreadWaiting(this,"queryTickettype",new Class[]{},new String[]{});
			waiting.task();
		}
	}
		...
		try {
			for (int i = 0; i < rights.size(); i++) {
				if (rights.get(i).getRightMethod().equals("queryMethod")){
					...
					int count = iEpdTickettype.queryCountByCustom(CommFinal.organize.getOrganizeSeq(),
							null);
					if (count>0){
						List<EpdTickettype> tickettypes = iEpdTickettype.queryPageByCustom(CommFinal.organize.getOrganizeSeq(),
								fCategory, start, limit);
						gridView.setDataList(tickettypes);
					}else{
						gridView.removeAll();
					}
	...
	public void addMethod(){
		EpdTickettypeEditUi editUi = new EpdTickettypeEditUi(this.getShell(),gridView,CommFinal.OPER_TYPE_ADD);
		editUi.open();
	}
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridView.getSelection()){
					EpdTickettypeEditUi editUi = new EpdTickettypeEditUi(this.getShell(), gridView, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
				}else{
	...
	public void deleteMethod(){
		try {
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		IEpdTickettype iEpdTickettype = new ImpEpdTickettype();
		    		iEpdTickettype.delete((List<EpdTickettype>) gridView.getSelections(),CommFinal.initConfig());
