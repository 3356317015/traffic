 ...
 * @author lcy
 */
public class EpdCargradeUi extends Composite {
	private Object obj;
	...
	public EpdCargradeUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	private void createDetail(){
		...
		columns.add(new GridColumn("车型代码","cargradeCode",120));
		columns.add(new GridColumn("车型名称","cargradeName",120));
	...
	public void queryMethod(){
		EpdCargradeFindUi findUi = new EpdCargradeFindUi(this.getShell());
		findUi.setfCargradeCode(fCargradeCode);
		findUi.setfCargradeName(fCargradeName);
		findUi.open();
		if (findUi.getBtnId()==1){
			fCargradeCode = findUi.getfCargradeCode();
			fCargradeName = findUi.getfCargradeName();
			ThreadWaiting waiting = new ThreadWaiting(this,"queryCargrade",new Class[]{},new String[]{});
			waiting.task();
		}
	}
	
	public void queryCargrade(){
		try {
			for (int i = 0; i < rights.size(); i++) {
				if (rights.get(i).getRightMethod().equals("queryMethod")){
					...
					int count = iEpdCargrade.queryCountByCustom(CommFinal.organize.getOrganizeSeq(),
							fCargradeCode, fCargradeName);
					if (count>0){
						List<EpdCargrade> cargrades = iEpdCargrade.queryPageByCustom(CommFinal.organize.getOrganizeSeq(),
								fCargradeCode, fCargradeName, start, limit);
						gridView.setDataList(cargrades);
					}else{
						gridView.removeAll();
					}
	...
	public void addMethod(){
		EpdCargradeEditUi editUi = new EpdCargradeEditUi(this.getShell(),gridView,CommFinal.OPER_TYPE_ADD);
		editUi.open();
	}
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridView.getSelection()){
					EpdCargradeEditUi editUi = new EpdCargradeEditUi(this.getShell(), gridView, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
				}else{
	...
	public void deleteMethod(){
		try {
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		IEpdCargrade iEpdCargrade = new ImpEpdCargrade();
		    		iEpdCargrade.delete((List<EpdCargrade>) gridView.getSelections(),CommFinal.initConfig());
