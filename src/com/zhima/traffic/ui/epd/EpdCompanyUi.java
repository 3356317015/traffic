...
/**
 * EpdCompanyUi概要说明：公司设置
 * @author lcy
 */
public class EpdCompanyUi extends Composite {
	private Object obj;
	...
	public EpdCompanyUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	private void createDetail(){
		...
		columns.add(new GridColumn("公司代码","companyCode",160));
		columns.add(new GridColumn("拼音代码","companySpell",160));
	...
	public void queryMethod(){
		EpdCompanyFindUi findUi = new EpdCompanyFindUi(this.getShell());
		findUi.setfCompanyCode(fCompanyCode);
		findUi.setfCompanyName(fCompanyName);
		findUi.setfCompanyType(fCompanyType);
		findUi.setfIndustry(fIndustry);
		findUi.open();
		if (findUi.getBtnId()==1){
			fCompanyCode = findUi.getfCompanyCode();
			fCompanyName = findUi.getfCompanyName();
		...
		try {
			for (int i = 0; i < rights.size(); i++) {
				if (rights.get(i).getRightMethod().equals("queryMethod")){
					...
					int count = iEpdCompany.queryCountByCustom(CommFinal.organize.getOrganizeSeq(),
							fCompanyCode, fCompanyName, fCompanyType, fIndustry);
					if (count>0){
						List<EpdCompany> companies = iEpdCompany.queryPageByCustom(CommFinal.organize.getOrganizeSeq(),
								fCompanyCode, fCompanyName, fCompanyType, fIndustry, start, limit);
	...
	public void addMethod(){
		EpdCompanyEditUi editUi = new EpdCompanyEditUi(this.getShell(),gridView,CommFinal.OPER_TYPE_ADD);
		editUi.open();
	}
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridView.getSelection()){
					EpdCompanyEditUi editUi = new EpdCompanyEditUi(this.getShell(), gridView, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
				}else{
	...
	public void deleteMethod(){
		try{
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		IEpdCompany iEpdCompany = new ImpEpdCompany();
					iEpdCompany.delete((List<EpdCompany>) gridView.getSelections(),CommFinal.initConfig());
