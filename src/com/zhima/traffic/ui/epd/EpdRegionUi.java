...
/**
 * EpdCityAreaUi概要说明：城市区域设置
 * @author lcy
 */
public class EpdRegionUi extends Composite {
	private Object obj;
	...
	public EpdRegionUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	private void createDetail(){
		...
		columns.add(new GridColumn("行政区划代码","administrationCode",120));
		columns.add(new GridColumn("拼音代码","regionSpell",110));
		columns.add(new GridColumn("省(市)","city",110));
		columns.add(new GridColumn("县","county",110));
		columns.add(new GridColumn("乡(镇)","towns",110));
	...
	public void queryMethod(){
		EpdRegionFindUi findUi = new EpdRegionFindUi(this.getShell());
		findUi.setfAdministrationCode(fAdministrationCode);
		findUi.setfRegionSpell(fRegionSpell);
		findUi.setfCity(fCity);
		findUi.setfCounty(fCounty);
		findUi.setfTowns(fTowns);
		findUi.open();
		if (findUi.getBtnId()==1){
			fAdministrationCode = findUi.getfAdministrationCode();
			fRegionSpell = findUi.getfRegionSpell();
			fCity = findUi.getfCity();
			fCounty = findUi.getfCounty();
			fTowns = findUi.getfTowns();
			ThreadWaiting waiting = new ThreadWaiting(this,"queryRegion",new Class[]{},new String[]{});
			waiting.task();
		}
	}
		...
		try {
			for (int i = 0; i < rights.size(); i++) {
				if (rights.get(i).getRightMethod().equals("queryMethod")){
					...
					int count = iEpdRegion.queryCountByCustom(CommFinal.organize.getOrganizeSeq(),
							fAdministrationCode, fRegionSpell, fCity, fCounty, fTowns);
					if (count>0){
						List<EpdRegion> cityAreas = iEpdRegion.queryPageByCustom(CommFinal.organize.getOrganizeSeq(),
								fAdministrationCode, fRegionSpell, fCity, fCounty, fTowns, start, limit);
						gridView.setDataList(cityAreas);
					}else{
						gridView.removeAll();
					}
	...
	public void addMethod(){
		EpdRegionEditUi editUi = new EpdRegionEditUi(this.getShell(),gridView,CommFinal.OPER_TYPE_ADD);
		editUi.open();
	}
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridView.getSelection()){
					EpdRegionEditUi editUi = new EpdRegionEditUi(this.getShell(), gridView, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
				}else{
	...
	public void deleteMethod(){
		try {
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		IEpdRegion iEpdRegion = new ImpEpdRegion();
		    		iEpdRegion.delete((List<EpdRegion>) gridView.getSelections(),CommFinal.initConfig());
