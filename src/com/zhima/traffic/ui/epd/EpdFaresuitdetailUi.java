...
/**
 * EpdFaresuitdetailUi概要说明：价套
 * @author lcy
 */
public class EpdFaresuitdetailUi extends Composite {
	private Object obj;
	//功能权限
	...
	public EpdFaresuitdetailUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	public Composite createFare(SashForm sashForm){
		...
		columns.add(new GridColumn("目的地","stationName",110));
		columns.add(new GridColumn("总价","fullFare",80));
	...
	public void queryMethod(){
		EpdFaresuitdetailFindUi findUi = new EpdFaresuitdetailFindUi(this.getShell());
	...
	public void fareMethod(){
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridRoute.getSelection()){
					EpdFaresuitdetailEditUi editUi = new EpdFaresuitdetailEditUi(this.getShell(), gridRoute, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
					queryFare();
				}else{
					MsgBox.warning(getShell(), "请选择修改的项。");
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridFare.getSelection()){
					EpdFaresuitdetailItemEditUi editUi = new EpdFaresuitdetailItemEditUi(this.getShell(), gridFare, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
				}else{
					MsgBox.warning(getShell(), "请选择修改的项。");
				}
	...
	public void deleteMethod(){
		try {
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		IEpdFaresuitdetail iEpdFaresuitdetail = new ImpEpdFaresuitdetail();
		    		iEpdFaresuitdetail.delete((List<EpdFaresuitdetail>) gridFare.getSelections(),CommFinal.initConfig());
