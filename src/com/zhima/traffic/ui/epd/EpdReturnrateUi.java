...
/**
 * EpdFaretypeUi概要说明：退票废票设置
 * @author lcy
 */
public class EpdReturnrateUi extends Composite {
	private Object obj;
	...
	public EpdReturnrateUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	private void createDetail(){
		...
		columns.add(new GridColumn("退票费率代码","returnrateCode",150));
		columns.add(new GridColumn("退票费率名称","returnrateName",150));
		columns.add(new GridColumn("上限分钟","timeStart",80));
		columns.add(new GridColumn("下限分钟","timeLimit",100));
		columns.add(new GridColumn("退票公式","formulaDesc",300));
	...
	public void queryMethod(){
		EpdReturnrateFindUi findUi = new EpdReturnrateFindUi(this.getShell());
		findUi.setfReturnrateCode(fReturnrateCode);
		findUi.setfReturnrateName(fReturnrateName);
		findUi.open();
		if (findUi.getBtnId()==1){
			fReturnrateCode = findUi.getfReturnrateCode();
			fReturnrateName = findUi.getfReturnrateName();
			ThreadWaiting waiting = new ThreadWaiting(this,"queryReturnrate",new Class[]{},new String[]{});
			waiting.task();
		}
	}
	
	public void queryReturnrate(){
		try {
			for (int i = 0; i < rights.size(); i++) {
				if (rights.get(i).getRightMethod().equals("queryMethod")){
					...
					int count = iEpdReturnrate.queryCountByCustom(CommFinal.organize.getOrganizeSeq(),fReturnrateCode, fReturnrateCode);
					if (count>0){
						List<EpdReturnrate> returnrates = iEpdReturnrate.queryPageByCustom(CommFinal.organize.getOrganizeSeq(),
								fReturnrateCode, fReturnrateCode, start, limit);
						gridView.setDataList(returnrates);
					}else{
						gridView.removeAll();
					}
	...
	public void addMethod(){
		EpdReturnrateEditUi editUi = new EpdReturnrateEditUi(this.getShell(),gridView,CommFinal.OPER_TYPE_ADD);
		editUi.open();
	}
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridView.getSelection()){
					EpdReturnrateEditUi editUi = new EpdReturnrateEditUi(this.getShell(), gridView, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
				}else{
	...
	public void deleteMethod(){
		try {
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		IEpdReturnrate iEpdReturnrate = new ImpEpdReturnrate();
		    		iEpdReturnrate.delete((List<EpdReturnrate>) gridView.getSelections(),CommFinal.initConfig());
