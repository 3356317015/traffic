 ...
 * @author lcy
 */
public class EpdCertificateUi extends Composite {
	private Object obj;
	...
	public EpdCertificateUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	private void createDetail(){
		...
		columns.add(new GridColumn("资质项类型","cerType",TraffFinal.ARR_CER_TYPE,120));
		columns.add(new GridColumn("资质项名称","cerName",120));
		columns.add(new GridColumn("资质项状态","cerStatus",TraffFinal.ARR_CER_STATUS,120));
		...
		gridView.bindRefresh(obj, "queryCertificate");
		FormData data = new FormData();
		data.top = new FormAttachment(0);
		data.left = new FormAttachment(0);
		data.bottom = new FormAttachment(100);
	...
	public void queryMethod(){
		EpdCertificateFindUi findUi = new EpdCertificateFindUi(this.getShell());
		findUi.setfCerType(fCerType);
		findUi.setfCerName(fCerName);
		findUi.setfCerStatus(fCerStatus);
		findUi.open();
		if (findUi.getBtnId()==1){
			fCerType = findUi.getfCerType();
			fCerName = findUi.getfCerName();
			fCerStatus = findUi.getfCerStatus();
			ThreadWaiting waiting = new ThreadWaiting(this,"queryCertificate",new Class[]{},new String[]{});
			waiting.task();
		}
	}
		...
		try {
			for (int i = 0; i < rights.size(); i++) {
				if (rights.get(i).getRightMethod().equals("queryMethod")){
					...
					int count = iEpdCertificate.queryCountByCustom(CommFinal.organize.getOrganizeSeq(),
							fCerType, fCerName, fCerStatus);
					if (count>0){
						List<EpdCertificate> certificates = iEpdCertificate.queryPageByCustom(CommFinal.organize.getOrganizeSeq(),
								fCerType, fCerName, fCerStatus, start, limit);
						gridView.setDataList(certificates);
					}else{
						gridView.removeAll();
					}
	...
	public void addMethod(){
		EpdCertificateEditUi editUi = new EpdCertificateEditUi(this.getShell(),gridView,CommFinal.OPER_TYPE_ADD);
		editUi.open();
	}
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridView.getSelection()){
					EpdCertificateEditUi editUi = new EpdCertificateEditUi(this.getShell(), gridView, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
				}else{
	...
	public void deleteMethod(){
		try {
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		IEpdCertificate iEpdCertificate = new ImpEpdCertificate();
		    		iEpdCertificate.delete((List<EpdCertificate>) gridView.getSelections(),CommFinal.initConfig());
