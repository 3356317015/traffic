...
public class LinerFareBar extends Composite {
	//控件面板
	private Composite composite;
	private int addWidth;
	//线路站点
	private List<LinerFareItem> fareItems = new ArrayList<LinerFareItem>();
	public LinerFareBar(Composite parent, List<Station> routeStations,int addWidth,int style) {
		...
		data = new FormData();
        data.top = new FormAttachment(0,5);
        data.left = new FormAttachment(lbIndex,50+addWidth/2);
		...
		data = new FormData();
        data.top = new FormAttachment(0,5);
        data.left = new FormAttachment(lbStationName,85+addWidth);
		...
		data = new FormData();
        data.top = new FormAttachment(0,5);
        data.left = new FormAttachment(lbFullFare,55+addWidth);
		...
		data = new FormData();
        data.top = new FormAttachment(0,5);
        data.left = new FormAttachment(lbBaseFare,50+addWidth);
		...
		data = new FormData();
        data.top = new FormAttachment(0,5);
        data.left = new FormAttachment(lbStationFare,40+addWidth);
	...
	private void createContent(List<Station> stations){
		if(null != stations && stations.size()>0){
			for(int i=0;i<stations.size();i++){
				LinerFareItem fareItem = new LinerFareItem(composite,i+1,addWidth,stations.get(i));
				fareItems.add(fareItem);
				if (i==0){
					fareItem.txtBaseFare.forceFocus();
					fareItem.txtBaseFare.selectAll();
				}
			}
	...
	public List getData(){
		...
		if (null != fareItems && fareItems.size()>0){
			for(int i=0;i<fareItems.size();i++){
				LinerFareItem fareItem =  fareItems.get(i);
	...
	public static void main(String[] args) {
		...
		new LinerFareBar(shell, routeStations, 0, SWT.BORDER);
