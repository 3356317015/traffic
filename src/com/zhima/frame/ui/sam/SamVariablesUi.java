 ...
 * @author lcy
 */
public class SamVariablesUi extends Composite {
	private Object obj;
	...
	public SamVariablesUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	private void createDetail(){
		...
		columns.add(new GridColumn("变量类型","variableType",CommFinal.ARR_VARIABLE_TYPE,160));
		columns.add(new GridColumn("变量代码","variableCode",160));
		columns.add(new GridColumn("变量名称","variableName",200));
		columns.add(new GridColumn("状态","status",CommFinal.ARR_VARIABLE_STATUS,120));
		...
		gridView.bindRefresh(obj, "queryVariables");
		FormData data = new FormData();
		data.top = new FormAttachment(0);
		data.left = new FormAttachment(0);
		data.bottom = new FormAttachment(100);
	...
	public void queryMethod(){
		SamVariablesFindUi findUi = new SamVariablesFindUi(this.getShell());
		findUi.setfVariableType(fVariableType);
		findUi.setfVariableCode(fVariableCode);
		findUi.setfVariableName(fVariableName);
		findUi.setfStatus(fStatus);
		findUi.open();
		if (findUi.getBtnId()==1){
			fVariableType = findUi.getfVariableType();
			fVariableCode = findUi.getfVariableCode();
			fVariableName = findUi.getfVariableName();
			fStatus = findUi.getfStatus();
			ThreadWaiting waiting = new ThreadWaiting(this,"queryVariables",new Class[]{},new String[]{});
			waiting.task();
		}
	}
		...
		try {
			for (int i = 0; i < rights.size(); i++) {
				if (rights.get(i).getRightMethod().equals("queryMethod")){
					...
					int count = iSamVariables.queryCountByCustom(CommFinal.organize.getOrganizeSeq(),
							fVariableType, fVariableCode, fVariableName, fStatus);
					if (count>0){
						List<SamVariables> samVariables = iSamVariables.queryPageByCustom(CommFinal.organize.getOrganizeSeq(),
								fVariableType, fVariableCode, fVariableName, fStatus, start, limit);
						gridView.setDataList(samVariables);
					}else{
						gridView.removeAll();
					}
	...
	public void addMethod(){
		SamVariablesEditUi editUi = new SamVariablesEditUi(this.getShell(),gridView,CommFinal.OPER_TYPE_ADD);
		editUi.open();
	}
		...
		for (int i = 0; i < rights.size(); i++) {
			if (thisMethodName.equals(rights.get(i).getRightMethod())){
				if(null!=gridView.getSelection()){
					SamVariablesEditUi editUi = new SamVariablesEditUi(this.getShell(), gridView, CommFinal.OPER_TYPE_UPDATE);
					editUi.open();
				}else{
	...
	public void deleteMethod(){
		try {
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		ISamVariables iSamVariables = new ImpSamVariables();
		    		iSamVariables.delete((List<SamVariables>) gridView.getSelections(),CommFinal.initConfig());
