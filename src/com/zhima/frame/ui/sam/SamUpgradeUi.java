...
/**
 * SamUserOnlineUi概要说明：在线升组文件查询
 * @author lcy
 */
public class SamUpgradeUi extends Composite {
	private Object obj;
	...
	public SamUpgradeUi(Composite parent, int style, List list) {
		super(parent, style);
	...
	private void createDetail(){
		...
		columns.add(new GridColumn("版本号","fileVer",180));
		columns.add(new GridColumn("文件名","fileName",200));
		columns.add(new GridColumn("路径","filePath",200));
		columns.add(new GridColumn("大小","fileSize",100));
		columns.add(new GridColumn("备注","remark",200));
		columns.add(new GridColumn("上传时间","createTime",200));
		...
		gridView.bindRefresh(obj, "queryUpgrade");
		FormData data = new FormData();
		data.top = new FormAttachment(0);
		data.left = new FormAttachment(0);
		data.bottom = new FormAttachment(100);
	...
	public void queryMethod(){
		ThreadWaiting waiting = new ThreadWaiting(this,"queryUpgrade",new Class[]{},new String[]{});
		waiting.task();
	}
		...
		try {
			for (int i = 0; i < rights.size(); i++) {
				if (rights.get(i).getRightMethod().equals("queryMethod")){
					...
					int count = iSamUpgrade.queryCountByCustom(CommFinal.organize.getOrganizeSeq());
					if (count>0){
						List<SamUpgrade> samUpgrades = iSamUpgrade.queryPageByCustom(CommFinal.organize.getOrganizeSeq(),start, limit);
						gridView.setDataList(samUpgrades);
					}else{
						gridView.removeAll();
					}
	...
	public void addMethod(){
		SamUpgradeEditUi editUi = new SamUpgradeEditUi(this.getShell(),gridView,CommFinal.OPER_TYPE_ADD);
		editUi.open();
	}
	...
	public void deleteMethod(){
		try {
			...
			if (checkIndex.length>0){
		    	...
		    	if(isdel == SWT.YES){
		    		ISamUpgrade iSamUpgrade = new ImpSamUpgrade();
					iSamUpgrade.delete((List<SamUpgrade>) gridView.getSelections(),CommFinal.initConfig());
